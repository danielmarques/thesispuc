...
As \textbf{árvores de decisão} são modelos de predição que utilizam os atributos de entrada para tomar decisões e desta forma inferir a classe de uma intância.
...
Seja \textit{k} o número de classes existentes no conjunto de dados, o \textit{Micro Precision} e o \textit{Micro Recall} multi-classe podem ser calculados pelas equações a seguir.
...
Neste caso as métricas são calculadas inividualmente para cada classe e no final é tirada uma média aritmética.
